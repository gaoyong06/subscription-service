/*
 ...
 Source Server         : 8.138.197.49
 Source Server Type    : MySQL
 Source Server Version : 80027 (8.0.27)
 Source Host           : 8.138.197.49:3306
...
CREATE TABLE `plan` (
  ...
  `app_id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '应用ID（关联api-key-service的app表）',
  `user_id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '开发者ID（用户ID，关联api-key-service的app.user_id）',
  ...
  `currency` varchar(10) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'USD' COMMENT '默认币种（用于兜底）',
  `period_type` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'MONTH' COMMENT '计费周期: DAY/MONTH/YEAR/FOREVER（UTC自然历）',
...
CREATE TABLE `plan_pricing` (
  ...
  `plan_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '套餐ID（关联plan表）',
  `app_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT '应用ID（冗余字段，通过plan_id关联，便于按app查询）',
  `country_code` varchar(10) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '国家代码（ISO 3166-1 alpha-2，如CN, US, DE等）',
...
CREATE TABLE `subscription_history` (
  ...
  `app_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT '应用ID（冗余字段，通过plan_id关联，便于按app统计和查询）',
  `start_time` datetime NOT NULL COMMENT '开始时间',
  `end_time` datetime NOT NULL COMMENT '结束时间',
  `status` varchar(20) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '状态',
  `action` enum('created','renewed','upgraded','paused','resumed','cancelled','expired','enabled_auto_renew','disabled_auto_renew') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '操作类型: created-创建, renewed-续费, upgraded-升级, paused-暂停, resumed-恢复, cancelled-取消, expired-过期, enabled_auto_renew-启用自动续费, disabled_auto_renew-禁用自动续费',
...
CREATE TABLE `subscription_order` (
  `order_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '订单号(业务订单号，与payment-service的order_id相同)',
  `payment_id` varchar(19) COLLATE utf8mb4_unicode_ci DEFAULT '' COMMENT '支付流水号(payment-service返回的payment_id，用于追溯支付记录)',
  `user_id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '用户ID',
  `plan_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '套餐ID',
...
CREATE TABLE `user_subscription` (
  ...
  `user_id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '用户ID',
  `plan_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '当前套餐ID',
  `app_id` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT '应用ID（冗余字段，通过plan_id关联，便于按app统计和查询）',
  `billing_anchor_day` tinyint NOT NULL DEFAULT '0' COMMENT '自然月/年锚点日1-31；DAY/FOREVER为0',
  `start_time` datetime NOT NULL COMMENT '开始时间',
  `end_time` datetime NOT NULL COMMENT '结束时间',
  `status` enum('active','expired','paused','cancelled') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'active' COMMENT '订阅状态: active-活跃(订阅有效中), expired-过期(订阅已过期), paused-暂停(用户主动暂停), cancelled-已取消(用户主动取消)',
  `order_id` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '' COMMENT '订单ID（关联subscription_order表）',
